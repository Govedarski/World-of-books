...
from my_project.library.views import ShowBooksToSendView

UserModel = get_user_model()


class ShowBooksToSendViewTest(django_test.TestCase):
    CREDENTIALS = {
        'username': 'user',
    ...
    def setUpClass(cls):
        ...
        cls.TARGET_URL = reverse('show_books_to_send')
    ...
    def test_show_books_to_send_when_user_have_books_to_send__expect_right_context_and_to_show_books(self):
        self._create_books(5, self.SECOND_USER)
        self._login()
        expected_books_to_show = Book.objects.filter(previous_owner=self.USER)

        response = self.client.get(self.TARGET_URL)

        self.assertEqual(ShowBooksToSendView.TITLE, response.context.get('title'))
        self.assertEqual(self.USER, response.context.get('owner'))
        self.assertQuerysetEqual(expected_books_to_show,
                                 response.context.get(ShowBooksToSendView.context_object_name))

    def test_show_books_to_send_when_user_have_no_books_to_send__expect_right_context_no_book_to_show(self):
        ...
        self.assertFalse(response.context.get(ShowBooksToSendView.context_object_name))
