...
class AcceptDeleteBookViewTest(django_test.TestCase):
    CREDENTIALS = {
        'username': 'user',
    ...
    def setUpClass(cls):
        ...
        cls.TARGET_URL = reverse('accept_delete_book',
                                 kwargs={'pk': notf.pk})
    ...
    def test_accept_d_book_view__when_valid_expect_book_change_owner_create_notf_and_redirect_to_next(self):
        book = self.BOOK
        book.ex_owners.add(self.USER)
        book.save()
        ...
        self.assertEqual(self.SECOND_USER, result_book.next_owner)
        self.assertEqual(self.USER, result_book.previous_owner)
        self.assertTrue(result_notf.is_answered)
        self.assertRedirects(response, reverse('show_books_on_a_way'))

    def test_accept_d_book_view__when_notf_do_not_have_book__expect__status_code_404(self):
        ...
        self._login(**self.SECOND_CREDENTIALS)
        target_url = reverse('accept_delete_book',
                             kwargs={'pk': notf_without_book.pk})

        response = self.client.get(target_url)
        self.assertEqual(404, response.status_code)

    def test_accept_d_book_view__when_notf_do_not_exist__expect__status_code_404(self):
        self._login(**self.SECOND_CREDENTIALS)
        target_url = reverse('accept_delete_book',
                             kwargs={'pk': 9999})
    ...
    def test_accept_d_book_view__when_notf_is_answered_allready__expect__status_code_403(self):
    ...
    def test_accept_d_book_view__when_no_authenticated_user__expect_redirect_to_login_with_next(self):
        response = self.client.get(self.TARGET_URL)
    ...
    def test_accept_d_book_view__when_user_is_not_recipient__expect_status_code_403(self):
        self._login()
        response = self.client.get(self.TARGET_URL)
        self.assertEqual(403, response.status_code)
